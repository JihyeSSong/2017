...
\caption{ Multiplicity distribution of accepted events in p--Pb collision in percentile. The each color presents the four intervals for the analysis.}
...
\noalign{\smallskip}
\end{tabular}
\caption{Number of analyzed events per multiplicity/centrality interval}\label{table:NCentralityEvent}
\end{table}
...
described in Ref.~\cite{cite:ALICEPerformance}. The primary tracks were chosen with the Distance of Closest Approach (DCA) to PV of less than 2 cm along the longitudinal direction (DCA$_z$)  and lower than 7$\sigma_r$ in the transverse plane (DCA$_r$), where $\sigma_r$ is the resolution of DCA$_r$. The 
...
selected with a DCA of less than 1.9~cm and with a DCA$_r$ to the PV greater than 0.015 cm. The $\Lambda$ daughter particles ($\pi$ and p) needs to have a DCA$_r$ to the PV greater than 0.06 cm, while the DCA between the two particles was required to be less than 1.4 cm. The cosine of the pointing angle ($\theta_\Lambda$, $\theta_\Xi$) and the radius of 
...
\caption{ TPC $\mathrm{d}E/\mathrm{d}x$ as function of transverse momentum in p--Pb collisions for total (Left) and selected first emitted $\pi$ in 3$\sigma$ (Right) }
\end{center}
\end{figure}
...
\caption{ TPC $\mathrm{d}E/\mathrm{d}x$ as function of transverse momentum in p--Pb collisions for total (Left) and selected second emitted $\pi$ in 3$\sigma$(bottom) }
\end{center}
\end{figure}
...
\caption{ TPC $\mathrm{d}E/\mathrm{d}x$ as function of transverse momentum in p--Pb collisions for total (Left) and selected last emitted $\pi$ in 3$\sigma$(bottom) }
\end{center}
\end{figure}
...
\label{fig:pPb:TPCp} 
\caption{ TPC $\mathrm{d}E/\mathrm{d}x$ as function of transverse momentum in p--Pb collisions for total (Left) and selected proton in 3$\sigma$(bottom) }
\end{center}
\end{figure}
...
\caption{ TPC $\mathrm{d}E/\mathrm{d}x$ as function of transverse momentum in Pb--Pb collisions for total (Left) and selected first emitted $\pi$ in 3$\sigma$ (Right) }
\end{center}
\end{figure}
...
\caption{ TPC $\mathrm{d}E/\mathrm{d}x$ as function of transverse momentum in Pb--Pb collisions for total (Left) and selected second emitted $\pi$ in 3$\sigma$ (Right) }
\end{center}
\end{figure}
...
\caption{ TPC $\mathrm{d}E/\mathrm{d}x$ as function of transverse momentum in Pb--Pb collisions for total (Left) and selected last emitted $\pi$ in 3$\sigma$ (Right) }
\end{center}
\end{figure}
...
\label{fig:PbPb:TPCp} 
\caption{ TPC $\mathrm{d}E/\mathrm{d}x$ as function of transverse momentum in Pb--Pb collisions for total (Left) and selected proton in 3$\sigma$ (Right) }
\end{center}
\end{figure}
...
Since the resonance decay products come from location which is indistinguishable from the PV, a significant combinatorial background is present. In order to extract \xis signal, it is necessary to reduce the combinatorial background as much as possible. For the \xis analysis, event mixing (EM) technique has been applied, by combining uncorrelated decay products  20 different events in p--Pb (5 different events in Pb--Pb). 

The events for the mixing have been chosen by applying the similar selection to minimize distortions due to different acceptances and to ensure a similar event structure, only tracks from events with similar vertex positions $z$ ($|\Delta z| <$ 1 cm) and track multiplicities $n$ ($|\Delta n|<$ 10) were taken.
...
~around the $\Xi^{*0}$ mass peak (Figure \ref{fig:sigpPbb} and \ref{fig:sigPbPbb}). These regions were used for all \pt intervals and multiplicity classes, because the background shape is reasonably well reproduced in these regions and the invariant-mass resolution of the reconstructed peaks appears stable, independently of \pt. The uncertainty on the normalization was estimated by varying the normalization regions and is included into the systematic uncertainty due to the signal extraction (Section\ref{sec:sys}).
...
\caption{\xis mass distribution as a function of \pt in each multiplicity classes in p--Pb collisions (top) and the different centrality classes in Pb--Pb (bottom). The mass values are obtained from fit of the Voigtian function.} 
 \label{fig:mass}
\end{center}
\end{figure}
...
\caption{The raw spectra of \xis obtained by integrating the Voigtian fit function for different multiplicities in p--Pb collisions (top) and Pb--Pb collisions (bottom). Only the statistical error is reported.} 
 \label{fig:rawyield}
\end{center}
\end{figure}
...
\caption{The geometrical acceptance and the reconstruction efficiency (A $\times$ $\epsilon$) for \xis in $-0.5<y_{\mathrm{CMS}}<0$. Only statistical uncertainties are shown.} 
